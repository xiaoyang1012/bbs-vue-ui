          ...
          <div class="author-name-meta" style="padding-left: 10px;">
            <div class="author-name">
              <a target="_blank" class="username" @click="routerUserCenter(data.createUser)">
                <span class="name" style="font-size: 17px;">{{ data.createdUserName }} </span>
                <img :src="require('@/assets/img/level/' + 'Lv1' + '.svg')" alt=""
                     @click.stop="routerBook" />
              </a>
            </div>
            <div class="meta-box" style="color: #8a919f">
            <span class="time" style="margin-right: 20px">
              {{ $utils.timestampFormat(data.createdAt) }}
            </span>
              <span class="views-count">
               {{ $t("common.read") + " " + data.viewCount }}
            </span>
            </div>
          </div>
        </div>
      </div>
      <div class="article-titleMap" v-if="data.pic">
        <img :src="data.pic" style="width: 100%;" />
      </div>
      <div class="article-content" style="width: 100%" v-if="data.detail.markdown">
        <mavon-editor
          :value="data.detail.markdown"
          :subfield="false"
          defaultOpen="preview"
          :toolbarsFlag="false"
          boxShadowStyle="0"
    ...
    getArticleById() {
      ...
      articleService.getArticleById(this.$route.params.id)
        .then(res => {
          ...
          // });
          this.$emit("initLabelIds", ['测试'], this.finish, res.data.createdUserName, this.$utils.toToc(res.data.detail.html));

          if (res.data.detail.html) {
            setTimeout(() => {
              // 设置标题目录追踪滚动高亮当前标题
